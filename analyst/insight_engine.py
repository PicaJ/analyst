...
每个 actionable_item 的 targets 数量上限 30 只，只要是"可用股票代码及公司信息"列表中与分析论点相关的股票都可以推荐，优先推荐产业链上下游或同板块中受益程度不同的标的。targets 必须按推荐强度从强到弱排列（最值得买入的排在最前面），target_reasons 也按相同顺序排列。
根据你的专业知识，推荐与投资论点最相关的 A 股标的，确保股票代码和公司名称准确。

⚠️ 风口研报识别（关键）:
很多新闻采用"风口研报"格式描述公司但不直接点名，例如:
- "这家公司6月起全面涨价" → 需根据上下文推断具体公司
- "某细分领域龙头订单已排至2030年" → 需识别该领域的A股龙头
- "布局电子铜箔关键赛道，已斩获样品订单" → 需找到对应的A股公司
对于这类隐含公司的新闻，你务必根据新闻描述的业务特征、行业地位、产品名称等线索，推断出最可能的A股公司并纳入推荐。这些通常是市场风口，价格尚未充分反映，投资价值最高。
...
- 推荐的股票必须与投资论点直接相关
- 每个 actionable_item 应推荐多只相关股票（2-30 只），涵盖产业链上下游或同板块不同环节，尤其是新闻中隐含提及但未直接点名的公司
- ⚠️ 特别关注"风口研报"类新闻: 标题含"风口研报"、"电报解读"、"金牌纪要"等前缀的新闻通常描述具体公司但不点名。你务必根据描述的业务特征、行业地位、产品参数等线索推断出对应的A股公司并纳入推荐，这些往往是价格尚未消化的市场风口
...
- evidence_ids 使用新闻序号 (如 "1", "2", "3")，对应新闻列表中的 [1], [2], [3]
...
def _format_news_list(nodes: List[ChainNode], max_items: int = 30) -> str:
    ...
    # 展示前面的新闻 (完整信息)
    # 使用序号 [1], [2]... 作为 ID，与 evidence_ids 引用一致
...
class InsightEngine:
    ...
    async def analyze_chain(self, chain: ClueChain) -> Dict[str, Any]:
        ...
        try:
            ...
            result["chain_significance"] = chain.significance
